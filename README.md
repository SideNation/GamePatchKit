...
빌드한 패치 데이터를 Supabase Storage 버킷에 올린다. 대상 버킷과 인증 정보는 환경 변수로 전달하며, 버킷이 없으면 공개 버킷으로 생성한다.
...
배포에는 Supabase Access Token을, 함수 호출에는 bucket만 있으면 된다. 포인터 테이블은 미리 준비해야 한다.
