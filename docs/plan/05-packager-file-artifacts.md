...
- [ ] 모든 artifact의 크기·SHA-256 재검증 → `dataVersion`·`releaseId` 계산 →
      manifest·build report 생성 → 전체 참조 검증

### incremental package

- [ ] 이전 manifest의 schema·packageId·releaseId·참조 검증
- [ ] 경로와 `fileHash`가 같은 파일은 기존 artifact 참조 재사용
- [ ] 추가·변경 파일은 group mode와 관계없이 개별 file artifact override로 생성
...
- [ ] `manifests/<releaseId>/manifest.json` 배치
- [ ] 추가·변경·삭제 목록, 생성 시각, 머신, source revision은 build report에만 기록

### 재현성·멱등성

- [ ] 임시 파일은 최종 output과 같은 filesystem에 만들고 검증 후 rename
- [ ] hash 경로에 다른 byte가 있으면 덮어쓰지 않고 실패
- [ ] 같은 release 재생성은 기존 결과를 검증하고 재사용
