...
- [ ] package별 root에 state·manifest cache·artifact cache·installation·staging을
      분리하고 package 사이의 local deduplication을 하지 않음
- [ ] `packages/<packageId>/{state,manifests,cache,installs,staging}` 고정 layout과
      각 디렉터리 책임 문서화
- [ ] `<runtimeRoot>/packages/<packageId>/state/package-state.json` 한 파일에
      `PackageState` 저장
- [ ] `state/package-state.lock` 경로를 사용할 수 있으나 파일 존재 여부가 아니라 OS
      file lock 또는 동등한 exclusive handle로 package writer 직렬화
- [ ] activation batch의 모든 staging을 검증된 immutable installation으로 rename한
      뒤 state를 한 번만 commit
- [ ] state 임시 파일을 최종 state와 같은 filesystem에 쓰고 flush한 뒤 OS별 원자적
...
- [ ] 실제 filesystem 기반 required-only 최초 설치, optional 후속 설치, unchanged
      optional 무다운로드 재연결, changed optional `stale` 전환
- [ ] 여러 group의 download·staging 완료 순서를 바꿔도 batch당 state replace가 한
      번뿐이며 한 group 실패 시 부분 활성화가 없는지 검증
- [ ] state 파일 없음·손상·미지원 schema 각각에서 trusted target 유무에 따른 복구 동작
- [ ] state write·flush·installation rename·atomic replace 각 지점의 중단 실패 주입과
      동시 writer 차단
