...
- [ ] config의 모든 glob pattern을 Core parser로 검증하고 전역 include → exclude →
      group matching 순서와 중복 group 오류를 filesystem 비의존 함수로 제공
- [ ] `ManifestValidator`의 filesystem 비의존 의미 검증
...
- [ ] RFC 8785 JCS·I-JSON 기반 직렬화: UTF-8, BOM·공백·trailing newline 없음,
      object property의 UTF-16 code unit 재귀 정렬과 JCS 문자열 escape
- [ ] JSON 숫자는 `-(2^53-1)`~`2^53-1`의 integer만 허용하고 field별 부호·범위는
      schema로 제한
- [ ] domain 배열 정렬: `groups[]` 이름, `files[]` 정규화 경로,
      `artifacts[]` content-addressed 경로, `parts[]` index, bundle `entries[]`
      정규화 경로
- [ ] 문자열 배열은 정규화한 UTF-8 byte의 ordinal 오름차순이며 JCS는 array 원소
      순서를 바꾸지 않음을 명시
...
- [ ] 최소 `single-file`, `multipart-file`, `bundle-entry` vector를 고정하고 optional
      group·혼합 compression branch도 포함
- [ ] vector별 입력 모델, canonical identity byte, 예상 `dataVersion`, canonical
      manifest byte와 예상 `manifestHash` 저장
- [ ] 정확한 byte 비교, parse→write 동일성, 배열 순서·참조·encoding 변형 negative
      vector 제공
- [ ] 11단계가 같은 vector에 raw test public key, 파생 `keyId`, 예상 canonical
      `manifest.sig` byte를 추가할 수 있는 디렉터리 계약 정의

### 경로·glob 순수 규칙

- [ ] Core는 filesystem을 조회하지 않고 전달받은 경로 문자열·pattern·후보 목록만
      처리한다. symlink·reparse point 판정과 source 경합 검증은 05 Packager 책임이다.
- [ ] canonical 상대 경로는 `/`와 NFC를 사용하고 절대 경로, 빈 segment, `.`·`..`,
      역슬래시와 NUL을 거부한다.
- [ ] NFC 정규화 후 UTF-8 byte가 같거나 `OrdinalIgnoreCase` 비교에서만 같아지는
      경로를 OS와 관계없이 중복으로 거부하고, 최종 목록은 정규화 UTF-8 byte의
      ordinal 오름차순으로 정렬한다.
- [ ] 자체 glob parser·matcher는 literal, segment 내부 `*`, 완전한 segment `**`만
      지원한다. `**`는 0개 이상 segment와 일치해 `**/*.json`이 root와 하위를 모두
      포함한다.
- [ ] 부분 `**`, 절대 pattern, 빈 segment, `.`·`..`, 역슬래시, NUL, `?`,
      character class, brace expansion, extglob과 negation은 검증 단계에서 거부한다.
- [ ] glob matching은 모든 OS에서 ordinal case-sensitive이며 locale·filesystem
      case 규칙에 의존하지 않는다.
- [ ] segment가 `.`으로 시작하면 숨김 경로로 정의한다. `*`와 `**`는 숨김 segment를
      소비하지 않고 대응 pattern segment가 literal `.`으로 시작할 때만 일치시킨다.
      Windows Hidden attribute는 사용하지 않는다.
- [ ] `include[]` OR → `exclude[]` OR(항상 우선, re-include 없음) → group
      `include[]` OR 순서로 적용한다. 둘 이상 group 일치는 오류, 미일치는 `default`다.
- [ ] table fixture에 `**`의 0·1·여러 segment, `/`·역슬래시, case, NFC, exclude
