...
> PRD 섹션: `dataVersion`과 `releaseId`, diff와 download plan, 프로젝트 책임
> `GamePatchKit.Core`, Runtime adapter contract

## 목표

Packager와 Runtime이 공유하는 순수 로직을 Core에 구현한다: streaming hash,
`dataVersion`·`releaseId` 계산, release diff, download plan, `ICompressionCodec`
contract.
...
- [ ] 입력: `dataVersion`, 각 파일의 artifact 참조, artifact type·경로·크기·SHA-256·
      압축 방식, bundle entry와 file part 순서
...
- [ ] 로컬 상태(경로 + 검증된 `fileHash`)와 목표 `files[]`를 비교한다
...
- 파일 내용 또는 group 의미가 바뀌면 `dataVersion`·`releaseId`가 모두 바뀐다.
- 같은 논리 상태에서 물리 배치만 바꾸면(compact 상황 모사) `dataVersion`은 유지되고
  `releaseId`만 바뀐다(검증 기준 9의 단위 수준).
