...
### stall 타임아웃 — 이 저장소에서 한다

SDK에는 타임아웃이 없다. `UnityWebRequest.timeout`을 설정하지 않으므로 끊긴 연결은 호출자 토큰이 없으면 영원히
기다린다. 소비자는 그래서 동기화 **전체**에 고정 시간 제한을 걸어 왔는데, 그 방식은 정상 다운로드가 제한을 넘는
순간 실패로 바뀌는 잠재 버그다.

옳은 형태는 "N초 동안 수신 바이트가 늘지 않으면 실패"다. 전체 소요 시간과 무관하고 진짜 끊긴 연결만 잡는다.
이 작업으로 `SendAsync`에 수신 바이트 관측이 생겼으므로 SDK 안에서는 몇 줄이다. 소비자 쪽에서 하려면
진행률 스냅샷을 감시하는 워치독과 토큰 연결을 소비자마다 짜야 한다.

설정값 하나(`StallTimeout`)가 생기므로 `minimal-implementation` 규칙상 정당화가 필요하다: **모든 소비자가 같은
것을 원하고, 바이트 단위 관측은 SDK만 갖고 있다.** 별도 설계 문서로 진행한다.
