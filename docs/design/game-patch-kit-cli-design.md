...
| `ArtifactWriter` | File writer | 아카이브·파일 객체 쓰기, zstd 적용, stored size와 SHA-256 계산 |
...
internal sealed record WrittenArchive(string Name, long PayloadSize, long StoredSize, string Checksum);
...
| 이전과 현재의 packing/compression이 다르지만 버전 동일 | 빌드 중단 후 그룹 버전 변경 안내 |
...
- 수행:
  ...
  - 정렬 순서대로 offset·length와 `<그룹 버전>.0` 버전을 만든다.
...
- 수행:
  ...
  - 변경 파일은 이전 리비전 + 1, 신규 파일은 리비전 0으로 객체를 쓴다.
...
- 수행:
  - 세 RID를 각각 해당 운영체제의 네이티브 환경에서 restore, test, publish, smoke run 한다.
...
| 13. 세 RID 실행 | `PublishAndSmoke_<rid>` | 네이티브 환경 검증 |

테스트 이름은 구현 시 실제 대상 타입에 맞춰 조정할 수 있지만, 각 완료 조건을 검증하는 시나리오는 삭제하지 않는다.

## 18. 완료 정의

- PRD 완료 조건 13개가 모두 자동 테스트 또는 RID별 smoke 검증에 연결되어 있다.
- `dotnet restore`, `dotnet build`, `dotnet test`가 성공한다.
