...
- `--output`은 `--source` 바깥에 둔다. 산출물이 입력 파일 탐색이나 워킹 트리 상태에 섞이는 경우를 막기 위한 최소 제약이다.
...
| `ArtifactWriter` | File writer | 아카이브·파일 객체 쓰기, zstd 적용, stored size와 SHA-256 계산, 아카이브 엔트리별 offset·length 산출 |
| `IncrementalPrecondition` 검사 | `BuildCommand` 내부 절차 | 이전 커밋 도달 가능 여부, 승계 산출물 존재 여부, `packing`·`compression` 동일 여부 확인 |
| `BuildSummary` / `GroupBuildSummary` | Output model | 그룹별·전체 빌드 요약 데이터 |
...
internal sealed record WrittenArtifact(string Name, long StoredSize, string Checksum);
...
internal sealed record BuildSummary(IReadOnlyList<GroupBuildSummary> Groups);
...
인자 파싱
  ...
  → 이전 sourceCommit 대비 changed path 계산
  → 그룹별 전체 빌드 또는 증분 빌드
  → 그룹/엔트리 ordinal 정렬
...
| --- | --- | --- |
| 이전 커밋 도달 | 이전 매니페스트의 `sourceCommit`이 현재 저장소에 **있다** (`git cat-file -e <commit>^{commit}`) | 중단. rebase·force push·shallow clone을 원인으로 알리고 그룹 버전을 올려 다시 빌드하도록 안내 |
| 승계 산출물 존재 | 이전 매니페스트가 참조하는 아카이브·파일 객체가 `--output`에 **실재한다** | 중단. 없는 경로를 나열 |
| 설정 동일 | 그룹 버전을 그대로 둔 그룹은 `packing`·`compression`이 이전 매니페스트와 **같다** | 중단. 바뀐 그룹 id와 함께 그룹 버전을 올리도록 안내 |
| 아카이브 경로 미점유 | 이전 매니페스트에 없는 그룹은 대상 아카이브 경로가 **비어 있다** | 중단. 이미 배포된 아카이브를 다른 내용으로 덮어쓰는 상황임을 알림 |

승계 산출물 검사는 이전 매니페스트에서 실제로 이어받는 그룹만 대상으로 한다. 그룹 버전이 바뀌어 전체 빌드하는 그룹은 승계하지 않으므로 제외한다.
...
- 위 "아카이브 충돌" 검사를 통과한 뒤에는 대상 경로를 덮어쓴다. 앞선 빌드가 산출물을 일부 쓰고 `manifest.json` 직전에 실패한 경우, 같은 커밋으로 다시 실행하면 같은 이름에 같은 내용을 쓰게 되므로 덮어쓰기가 정상 복구 경로다.
- 매니페스트를 마지막에 기록하므로 중간 실패 시 `--output`에는 미참조 객체만 남는다. 정리는 제외 범위다.
...
- 수행:
  ...
  - 이전 `sourceCommit`이 현재 저장소에 있는지 `git cat-file -e <commit>^{commit}`으로 확인하고, 없으면 중단한다.
- 검증:
  ...
  - 이전 매니페스트의 커밋이 사라진 저장소(리셋 후 새 커밋)에서 빌드가 중단되고 `--output`이 변하지 않는다.
...
- 수행:
  ...
  - 이전 매니페스트에 없는 그룹인데 대상 아카이브 경로에 파일이 이미 있으면 중단한다.
- 검증:
  ...
  - YAML에서 뺐던 그룹을 같은 버전으로 되살리면 중단되고 기존 아카이브가 그대로 남는다.
- 완료 조건 연결: 1, 2, 5, 15
...
| PRD 완료 조건 | 대표 테스트 | 수준 |
| --- | --- | --- |
| 1. group 최초 빌드 | `Build_Group_FirstBuild_CreatesSingleArchive` | 통합 |
...
| 5. 그룹 버전 변경 | `Build_GroupVersionChangeRebuildsArchiveAndResetsRevisions` | 통합 |
...
| 16. 증분 전제 위반 중단 | `Build_MissingPreviousCommitFailsWithoutOutputChanges`<br>`Build_MissingInheritedArtifactFailsWithoutOutputChanges` | 통합 |

테스트 이름은 구현 시 실제 대상 타입에 맞춰 조정할 수 있지만, 각 완료 조건을 검증하는 시나리오는 삭제하지 않는다.

## 18. 완료 정의

- PRD 완료 조건 16개가 모두 자동 테스트 또는 RID별 smoke 검증에 연결되어 있다.
- `dotnet restore`, `dotnet build`, `dotnet test`가 성공한다.
- `win-x64`, `linux-x64`, `osx-arm64` 네이티브 환경에서 zstd 빌드가 실행된다.
- 동일 HEAD에서 다시 실행했을 때 새 산출물이 없고 매니페스트 바이트가 같다.
- 증분 전제 위반 네 가지가 모두 산출물을 쓰기 전에 중단되고 `--output`을 바꾸지 않는다.
