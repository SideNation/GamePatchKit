...
- 릴리스 태그 `v0.2.2`를 만든다. 태그는 CLI의 NuGet 배포 버전이라 기존 계보(`v0.2.1`)를 잇는다. UPM
  `package.json`의 `0.1.12`와 숫자가 다른 것은 두 버전 축이 원래 따로 움직이기 때문이다. `v0.1.12`로 태그하면
  이미 발행된 0.2.1보다 낮은 버전을 NuGet에 발행하게 된다.
