...
`gpk sync` 명령으로 Supabase Postgres의 릴리스 버전 포인터를 조회하고, 로컬 폴더가 그 세대와 다르면 공개 Storage에서 세대 매니페스트와 없는 산출물만 내려받아 로컬 트리를 게시된 세대로 맞춘다. 스케줄러(cron 등)가 주기 실행하는 명령과 사람이 강제로 실행하는 명령이 같은 멱등 one-shot이며, 배타 파일 락으로 겹침만 방지한다.
...
- 성공(동기화): `동기화 완료: releaseVersion=<N> (이전: <M|없음>), downloaded=<K>, reused=<R>, downloadedBytes=<B>` — 종료 코드 0
...
- 로컬 산출: `<output>/manifest.json`(세대 매니페스트 바이트 그대로), `<output>/archives/**`, `<output>/files/**`, `<output>/.gpk-sync.lock`(락 파일, 내용 없음, 삭제하지 않음)
...
| 책임 단위 | 종류 | 설명 |
| --- | --- | --- |
| `SyncCommand` | Command | 락 획득, 포인터 비교, 델타 계산, 다운로드 검증, 매니페스트 원자 교체 |
...
internal sealed record SyncSummary(
    SyncOutcome Outcome,
    int? PointerVersion,   // SkippedBecauseLocked면 null
    int? PreviousVersion,  // 로컬 manifest.json이 없던 첫 동기화면 null
    int DownloadedCount,
...
| --- | --- |
| 새 파일 3개(src) | `upload`가 확립한 명령 단위 파일 분리(Command/Settings/Storage)와 동일한 배치. 한 파일로 합치면 기존 구조 관례를 깬다. |
| 새 타입 8개 | 각각 `upload`의 대응물(UploadArguments/UploadSettings/UploadSettingsResolver/IUploadStorage/SupabaseUploadStorage/UploadSummary)과 1:1 대칭이고, `SyncOutcome`만 추가 — 락 겹침을 예외가 아닌 정상 결과로 표현하기 위해 필요하다. |
...
9. 전부 성공하면 `ManifestStore.WriteBytesAtomically`로 세대 매니페스트 바이트를 그대로 `manifest.json`에 원자 교체 — 이 시점에 로컬 세대가 전환된다.
10. `SyncSummary` 출력 후 종료 코드 0.

실패 지점 어디서든: 기존 `manifest.json`이 가리키던 세대는 온전히 남고, 내려받다 만 임시 파일은 참조되지 않는다.
