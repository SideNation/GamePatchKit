...
| `releaseId` | `dataVersion`을 구성하는 물리 artifact 배치의 digest |
| channel | 환경이 사용할 `releaseId`와 `dataVersion`을 가리키는 외부 pointer |
...
- 일반 filesystem cache·staging·current pointer
...
설정은 versioned YAML 한 파일로 관리하고 JSON Schema로 검증한다.
...
| `releaseId` | 물리 artifact 배치 digest |
...
- identity와 signature는 canonical JSON 원본을 기준으로 한다.
...
- `dataVersion`
- 각 파일의 artifact 참조
- artifact type, 경로, 크기, SHA-256과 압축 방식
- bundle entry와 file part 순서

`releaseId` 필드 자체는 계산 입력에서 제외한다.

결과:

- 파일 내용이나 group 의미가 바뀌면 `dataVersion`과 `releaseId`가 바뀐다.
- 같은 데이터를 compact하면 `dataVersion`은 유지되고 `releaseId`만 바뀐다.
- 같은 데이터를 다른 압축으로 패키징해도 `dataVersion`은 같다.
...
9. 물리 배치에서 `releaseId`를 계산한다.
10. manifest와 build report를 생성하고 전체 참조를 검증한다.

### incremental package

1. 이전 manifest의 schema, packageId, releaseId와 참조를 검증한다.
2. 현재 source의 최종 파일 목록과 `fileHash`를 이전 `files[]`와 비교한다.
3. 경로와 `fileHash`가 같으면 기존 artifact 참조를 재사용한다.
4. 추가·변경 파일은 group mode와 관계없이 개별 file artifact override로 만든다.
5. 삭제 파일은 새 `files[]`에서 제외한다.
6. 새 manifest는 patch chain이 아니라 현재 최종 상태 전체를 기록한다.
7. 새 artifact와 재사용 참조를 검증한 뒤 새 버전을 계산한다.

incremental package는 기존 bundle을 수정하거나 동일 경로에 다시 업로드하지 않는다.

### compact

1. source release와 대상 bundle group을 실행 시작 시 고정한다.
...
8. `dataVersion`은 유지하고 새 `releaseId`를 계산한다.
9. 새 bundle과 manifest를 불변 경로에 생성한다.
...
1. 신뢰하는 channel 또는 서버 응답에서 `packageId`, `dataVersion`, `releaseId`를 받는다.
2. 목표 manifest와 signature를 검증한다.
3. 로컬 상태와 목표 `files[]`로 download plan을 만든다.
4. adapter를 통해 누락 artifact를 content-addressed cache에 받는다.
5. artifact hash와 압축 해제 결과를 검증한다.
6. 목표 파일을 staging에 구성하고 크기와 `fileHash`를 검증한다.
7. 모든 필수 group이 준비되면 current release pointer를 원자적으로 교체한다.
8. 취소·실패·프로세스 종료 시 검증된 cache를 보존하고 다음 실행에서 이어받는다.
9. 활성화 실패 시 이전 current release를 계속 사용한다.
...
- `IRuntimeStorage`: content-addressed cache와 staging stream을 제공하고 current pointer를
  원자적으로 조회·교체한다.
...
- cache와 staging은 일반 filesystem에 구현한다.
- current pointer 교체는 해당 OS에서 가능한 원자적 filesystem 연산을 사용한다.
...
- hash 경로 artifact와 release manifest는 immutable 장기 cache
...
- 같은 source, 설정과 이전 manifest는 같은 artifact와 manifest를 생성한다.
...
2. config·manifest JSON Schema와 canonicalization 규칙을 고정한다.
3. Core의 hash, identity, diff, download plan과 `ICompressionCodec`을 구현한다.
