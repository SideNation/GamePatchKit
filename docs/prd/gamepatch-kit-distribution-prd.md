...
4. 아래 "데이터 복원"대로 `<output>/data`에 원본 트리를 만든다. 데이터를 쓰는 쪽이 CLI가 아니라 그 머신의 다른 프로세스이므로, 소비자가 매니페스트를 해석하지 않고 파일을 바로 읽을 수 있어야 한다.
5. 모든 산출물을 받아 검증하고 해제까지 끝낸 뒤에야 `manifest.json`을 원자적으로 교체한다. 중단되면 이전 세대가 그대로 남는다.
6. 읽기용 publishable key만 쓴다. secret key를 소비 머신에 두지 않는다.

같은 폴더에 대한 동시 실행은 배타 파일 락으로 하나만 진행한다. 자세한 것은 [`docs/cli/sync.md`](../cli/sync.md)를 따른다.

### 데이터 복원

매니페스트를 받은 소비자는 다음 순서로 파일 트리를 만든다. 서버와 클라이언트가 같은 구현을 쓰고, CLI 미러는 이것을 디스크에 그대로 수행한다.
...
4. 복원한 파일의 자리는 `<그룹 id>/<엔트리 path>`다. 그룹 id가 데이터 루트 기준 폴더 경로이므로 복원 결과는 빌드에 쓴 원본 트리와 같은 배치가 된다.
